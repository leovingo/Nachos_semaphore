...
int main()
{
	...
	SpaceId si_input, si_output, si_sinhvien, si_result,si_sinhvien_2;	// Bien id cho file
	int SLTD;	// Luu so luong thoi diem xet
	char c_readFile;	// Bien ki tu luu ki tu doc tu file
	char sv; //bien de luu sinh vien
	int temp;
	int flag;
	...
	while(SLTD--)
	{
		...
		si_sinhvien_2 = Open("sinhvien.txt", 0);
		while(1)
		{
			...
			while (1)
			{
				if(Read(&sv, 1, si_sinhvien_2) < 1)
				{
					break;
				}
				if (sv != 32)
				{
					Write(&sv, 1, si_output);
				}
				else
					break;
			}	
